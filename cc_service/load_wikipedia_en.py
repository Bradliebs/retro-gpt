"""Load full English Wikipedia into the memory bank.

Sibling of load_wikipedia.py — same logic, but points at the full English
Wikipedia dump (`20231101.en`) instead of the Simple English dump
(`20231101.simple`). Use this when the Simple corpus has been exhausted
and you need more tokens / domain coverage to break a training plateau.

Usage:
    python load_wikipedia_en.py [--max-articles N] [--skip-articles N] \
                                [--min-chars 80] [--batch-size 50]

Note: the full English dataset is ~20GB compressed on first download,
and has ~6.7M articles. Bound with --max-articles unless you really mean
to load all of it.
"""
...
def main():
    ...
    # Load dataset
    console.print("[cyan]Downloading full English Wikipedia (20231101.en)...[/cyan]")
    console.print("(First run downloads ~20GB; cached after that)")
